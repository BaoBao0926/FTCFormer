...
import math
from .tcformer_layers import Block, TCBlock, OverlapPatchEmbed, CTM, Conv_for_dict
...
class TCFormer(nn.Module):
    def __init__(
            ...
            return_map=False,
            Cmerge=True, FDPC_KNN=True, if_WSN=True, k_WSN=5, downsampling='CTM',
            **kwargs
    ):
        super().__init__()
        ...
        self.k = k
        if_CTM = True if downsampling=='CTM' else False
        ...
        for i in range(1, num_stages):
            if downsampling=='CTM':
                ctm = CTM(sample_ratios[i-1], embed_dims[i-1], embed_dims[i], k, Cmerge=Cmerge, FDPC_KNN=FDPC_KNN, if_WSN=if_WSN, k_WSN=k_WSN)
            else:
                ctm = Conv_for_dict(in_channel=embed_dims[i-1], out_channel=embed_dims[i], downsampling=downsampling)

            block = nn.ModuleList([TCBlock(
                dim=embed_dims[i], num_heads=num_heads[i], mlp_ratio=mlp_ratios[i], qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + j], norm_layer=norm_layer,
                sr_ratio=sr_ratios[i], if_CTM=if_CTM)
                for j in range(depths[i])])
            norm = norm_layer(embed_dims[i])
            cur += depths[i]
...
class TCFormer_backup(nn.Module):
    def __init__(
            self, img_size=224, in_chans=3, embed_dims=[64, 128, 256, 512],
            num_heads=[1, 2, 4, 8], mlp_ratios=[4, 4, 4, 4], qkv_bias=False,
            qk_scale=None, drop_rate=0., attn_drop_rate=0., drop_path_rate=0.,
...
class tcformer_light(TCFormer_backup):
...
class tcformer(TCFormer_backup):
    def __init__(self, **kwargs):
        super().__init__(
            embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
...
class tcformer_large(TCFormer_backup):
    def __init__(self, **kwargs):
        super().__init__(
            embed_dims=[64, 128, 320, 512], num_heads=[1, 2, 5, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True,
