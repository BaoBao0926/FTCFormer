...
def cluster_and_merge_flops(num_tokens, dim, k, FDPC_KNN=True, if_WSN=True):
    print(f'please note here, in the tcformer_module.tcformer_utils.cluster_and_merge_flops, FDPC_KNN and if_WSN is set as True')
    flops = 0
    flops += num_tokens * num_tokens * dim  # distance matrix
    if FDPC_KNN:
        flops += 2*num_tokens * num_tokens    # local density of Fuzzy KNN
    else:
        flops += num_tokens * k                 # local density
    if if_WSN:
        flops += num_tokens * num_tokens * k    # WSN
    flops += num_tokens * num_tokens        # distance indicator
    flops += num_tokens * dim               # token merge
    return flops
